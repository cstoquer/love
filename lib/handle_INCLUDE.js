...
module.exports = function (source, components, scriptDir, isDevMode) {
	...
	function handle_INCLUDE(source) {
		...
		for (var i = 0; i < includes.length; i++) {
			...
			var fileContent = fs.readFileSync(path.join(process.cwd(), scriptDir, filePath), { encoding: 'utf8' });
			...
			try {
				...
				throw new Error('in file ' + scriptDir + '/' + filePath + '\n' + e.message);
			}
